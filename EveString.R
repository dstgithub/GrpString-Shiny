...
function(eveName.file, eveName.vec, char.vec){
   ...
   if (string_lengths_sum > eveName.sp_num_sum){
      warning <- 'Warning: One or more event names are not replaced by characters. Please check the output vector and your conversion key.'
      return(c(warning, "", char_to_string.vec))
